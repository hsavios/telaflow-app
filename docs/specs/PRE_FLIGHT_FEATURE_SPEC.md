...
O fluxo normativo do produto é **Cloud → Pack → Player → binding local → Pre-flight → `ready` → `executing`** (PRODUCT_SPEC; ARCHITECTURE_SPEC §11–12, §14.0). O Pre-flight é o **gate** que transforma ficheiros numa pasta e um Pack em disco em **decisão operacional explícita**: **pode** ou **não pode** ir ao ar com **risco conhecido**. Esta spec **não** redefine o Pack nem o editor — fixa **o que** o Player **deve** verificar, **como** classificar resultados e **como** isso **move** a máquina de estados.
...
O PRODUCT_SPEC promete **confiabilidade operacional** e **pre-flight explícito** no MVP; a ARCHITECTURE_SPEC trata o Pre-flight como **componente arquitetural**, não como tela opcional. Sem Pre-flight disciplinado, o Player **cai** no improviso de palco que o produto **existência** para eliminar: telão a falhar **sem** diagnóstico partilhado entre operador e suporte.
...
| **Mídia obrigatória** | Garantir que o **manifesto** foi **cumprido** na workspace — ficheiros **presentes** e **coerentes** com o esperado. |
...
- **Reexecução:** sempre que **invalidação** ocorrer (secção 17) ou o operador **solicitar** “Executar checagens novamente”.
...
Ver secção **17** — resumo: mudança de binding, substituição de ficheiro, novo Pack, alteração de workspace, ou **pedido** explícito do operador.
...
| Qualquer onde run é possível | Run deteta **fatal** (secção 13) | **`blocked`** |
...
A engine é **agrupada** em **cinco** grupos **estáveis**. Cada grupo produz **itens** com severidade **OK**, **aviso** ou **bloqueante** (secção 12). A **consolidação** (secção 13) agrega **todos** os grupos.

| Grupo | Objeto de análise |
|-------|-------------------|
| **G1 — Integridade do Pack** | Artefato em disco, ficheiros internos, schema, assinatura, legibilidade. |
| **G2 — Licença** | Presença, assinatura, tempo, escopo, relógio local. |
| **G3 — Mídia** | Manifesto vs binding vs ficheiros na workspace. |
...
**Princípio:** falhas **cedo** nos grupos **G1–G2** podem **abreviar** trabalho **útil** dos grupos seguintes (secção 14), mas **resultado** **deve** **registar** **claramente** o **motivo** primário (suporte).

## 6.1 Modelo conceitual de um item de check

Cada **resultado** de verificação **atómica** (uma linha na UI, um registo na engine) **deve** ser tratado como **entidade estável**, **não** texto solto. Isto **uniformiza** engine, UI, logs e suporte.
...
| **`group`** | Sim | Um de **G1 … G5** (secção 6). |
| **`severity`** | Sim | **OK** \| **aviso** \| **bloqueante** (secção 12). |
| **`fatality`** | Quando `severity` = bloqueante | **recuperável** \| **fatal** \| `null` / omitido se não for bloqueante. |
| **`message`** | Sim | Texto **humano** para o operador (microcopy, secção 16). |
...
| Check | Descrição |
|-------|-----------|
| **Ficheiros esperados presentes** | Lista canónica de ficheiros do Pack (ex.: `pack.json`, `event.json`, manifestos, licença, assinatura) **existe** e é **lida**. |
...
| **Ficheiros legíveis** | Leitura **sem** erro de permissão ou I/O **nos** componentes do Pack. |
| **Estrutura coerente** | Parse de JSON **bem formado**; referências **internas** entre ficheiros do Pack **existem** (ex.: ids de export). |

## 7.2 Severidade sugerida

| Situação | Severidade | Nota |
|----------|------------|------|
| Ficheiro obrigatório do Pack **ausente** | **Bloqueante** — tipicamente **fatal** → **`blocked`** | Não há Pack válido. |
...
| JSON **malformado** em ficheiro crítico | **Bloqueante** **fatal** | |
| Ficheiro **extra** não assinado (se política) | **Aviso** | Não impede se política MVP ignorar. |
...
| Check | Descrição |
|-------|-----------|
| **Obrigatória presente** | Cada `media_id` **obrigatório** no manifesto tem **binding** e ficheiro **existente**. |
...
| **Duplicidade suspeita** | Dois `media_id` distintos apontam para o **mesmo** ficheiro — pode ser **intencional**; **avisar**. |
| **Ficheiro ilegível** | Permissão negada, volume desmontado, **smoke** open/read **falha**. |
...
1. **Presença** — ficheiro referenciado pelo binding **existe** no caminho resolvido.  
2. **Leitura mínima** — operação **I/O** mínima que prove **legibilidade** (ex.: abrir handle, ler **primeiros** bytes ou metadata **leve** conforme spec técnica derivada) — **não** ler o ficheiro inteiro **nem** exigir decode completo.  
3. **Extensão / tipo declarado** — coerência com o **manifesto** / `MediaRequirement` (extensão ou regra equivalente).

**Não** entra no MVP: **decode** profundo de vídeo/áudio, validação de **codec** interno, **duração** exacta por análise de stream, **resolução** por frame decode, **checksum** de ficheiro completo **salvo** ADR. Isto **evita** **creep** de complexidade e tempos **imprevisíveis** (secção 14.3). Evolução para checks **mais pesados** **exige** **ADR**, **timeouts** explícitos e **progresso** na UI.
...
| **Leitura de ficheiros** | Teste **representativo** (não precisa ler **todo** o binário de vídeo — **head**/metadata conforme spec técnica derivada). |
...
| Segundo ecrã **ausente** (se exigido) | **Aviso** ou **bloqueante** por política — MVP: **aviso** |
...
- **Bloqueante recuperável:** operador **pode** resolver **na** máquina local (binding, permissões, ficheiro errado) — consolidado leva a **`preflight_failed`**.  
...
- **Visualmente** (secção **15.5**): muitos avisos **não** devem **imitar** bloqueantes (cor, ícone, densidade de alarme) — o operador **deve** distinguir **claramente** “posso ir ao ar com riscos comunicados” de “**não** posso até corrigir bloqueantes”.
...
Após **uma** execução completa (ou **ciclo** definido na secção 17):

1. Executar grupos na **ordem** da secção **14** (com **atalhos** seguros se aplicável).  
...
- Cada **grupo** **G1…G5** **deve** ter **tempo operacional aceitável** definido em **implementação** (limites **por** check ou **por** grupo), com **especial atenção** a **G3** (muitos ficheiros, vídeos grandes, **shares** de rede lentos, disco externo).  
...
- Checks **longos** (**varrimento** de mídia, I/O lento) **devem** **reportar** **progresso** na UI (ficheiro **N** de **M**, grupo **G3** em curso — UI_SPEC §16.4), **não** **congelar** o ecrã **sem** feedback.  
...
Cada item **deve** ter **código** estável (`PACK_SIG_INVALID`, `MEDIA_REQUIRED_MISSING`, …) alinhado ao campo **`code`** do modelo **§6.1** — **partilhável** com suporte; pode viver em **shared-contracts** como enum (ARCHITECTURE_SPEC §21.4 — **sem** lógica de negócio, só **códigos**).
...
- Substituição de ficheiro **detetada** (watch **ou** reabertura de diálogo de mídia — política de implementação).  
...
- **Otimização futura** (ADR): subset de grupos (ex.: **só** **G3** após mudança de binding) — **só** **válida** se **respeitar** **§13.6** (registo de grupos **omitidos** / **reconsolidação** explícita) e **manter** **timestamps por grupo** para o que **correu**, **sem** **quebrar** a **norma** de **transparência** para suporte.

---

# 18. Relação com binding de mídia

- O grupo **G3** **consome** o **binding atual**; **não** há cache **silencioso** de paths **obsoletos** face ao ficheiro guardado.  
...
- **UI:** após alterar ficheiro, **CTA** “Executar checagens” **visível** — **não** assumir `ready` **automático** **sem** run **completado** (política: auto-run **após** debounce **é** **permitida** **se** **transição** de estado **for** **sempre** **explícita** na UI).
...
- **Não** spammar **por** check OK **individual** em ficheiro **salvo** salvo modo **debug**.
...
**Qualquer** bloqueante **no último consolidado** (secção 13).
...
- **Não** é **reclassificado** como “falha de pre-flight” **retroactivamente** — é **incidente de execução** (ficheiro **removido** a meio, etc.).  
...
6. **Saltar** G1/G2 **silenciosamente** **sem** registo **SKIPPED**.  
...
- **Hash** de ficheiro vs manifesto (se Cloud passar **hash** esperado).  
