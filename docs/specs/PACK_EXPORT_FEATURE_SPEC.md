...
**Escopo:** comportamento funcional e **contrato** do **artefato exportado** — **sem código**, **sem DDL**, **sem algoritmos criptográficos detalhados**. Schemas JSON canônicos, formatos exatos de licença e políticas numéricas de timeout pertencem a **`shared-contracts`** e especificações derivadas, **desde que** obedeçam a este documento e à **ARCHITECTURE_SPEC**.
...
| **Serializar dados consistentes** | Emitir JSON (e artefatos associados) que **validam** contra os schemas canônicos em `shared-contracts` **antes** de assinar; **sem** campos “soltos” fora do contrato. |
...
| **Arquivo ZIP de arquivos sem governação** | Sem `pack.json`, sem schema, sem assinatura, o Player **não** tem **norma** para rejeitar adulteração nem para versionamento. |
...
- O usuário tem **permissão** de exportação na organização (papel / política — detalhe em modelo de autorização, fora desta spec).  
...
- A operação de exportação **completa** gera `export_id`, artefatos versionados e **registro** de auditoria (§20).

## 4.2 O que precisa estar válido **antes**

- **Roteiro:** lista ordenada de Scenes **persistida** (`sort_order` canônico — EVENT_EDITOR §5.1); cada Scene **exportável** em estado que **não** viole regras bloqueantes.  
...
A composição **normativa** alinha-se a [ARCHITECTURE_SPEC.md](./ARCHITECTURE_SPEC.md) §9.1. Nomes de arquivo podem ajustar-se em spec derivada **desde que** os **papéis** abaixo permaneçam **distinguíveis** e o `pack.json` permaneça **raiz semântica**.
...
| **`branding.json`** | Snapshot do **BrandTheme** **resolvido** (tokens e regras aplicáveis ao Player — **não** editor gráfico livre). | **Obrigatório** se o evento tiver branding aplicável; se o produto definir tema **mínimo por padrão**, contém esse **default resolvido**. |
| **`media-manifest.json`** | Lista canônica de **MediaRequirement** serializados (`media_id`, obrigatoriedade, tipos, papéis) — contrato para binding local. | **Obrigatório** (pode ser array vazio **só se** o schema e o evento **permitirem** zero slots — política de produto; na prática típica há **≥1** entrada). |
...
| **`signature.sig`** | Assinatura sobre manifesto de arquivos contratuais ou payload canônico (§16). | **Obrigatório** |

**Nota sobre `scenes.json`:** a linha base **serializa** as Scenes **dentro** de `event.json` para **um** grafo de evento **coerente**. Um arquivo **`scenes.json`** separado **só** é admissível com **ADR** e **equivalência** contratual com o schema canônico (evitar duas fontes de verdade **sem** governação).
...
## 6.1 Ordem canônica de arquivos e de serialização para assinatura

A ordem abaixo é **normativa** para: (1) **construção** do manifesto de arquivos cuja lista de hashes alimenta a **assinatura**; (2) **verificação** no Player (loader / pre-flight **G1**); (3) **debug** e ferramentas que comparem Packs.
...
- **`signature.sig` entra por último** na lista **lógica** de “arquivos do Pack” para documentação e tooling; **não** é incluído no **payload** que a própria assinatura cobre (a assinatura **cobre** os artefatos 1–6 e/ou o manifesto derivado — §16).  
- Qualquer **arquivo contratual adicional** futuro (ADR) **insere-se** na ordem **com posição documentada** (nunca “no fim sem regra”).  
- **Serialização canônica** de cada JSON para cálculo de hash **deve** seguir §6.2 (encoding, newlines, omissão vs `null` — §6.3).

## 6.2 Codificação normativa dos arquivos JSON do Pack

Todos os artefatos **JSON** do Pack **devem** ser:

- **UTF-8** codificação de caracteres **exclusiva**;  
- **Sem BOM** (byte order mark **proibido**);  
- **Newline consistente:** **LF** (`\n`) como terminador de linha **em todo** o arquivo — **não** misturar CRLF/LF no mesmo artefato.
...
- **`null` explícito** **não** é preferido **salvo** o **JSON Schema** canônico desse artefato **exigir** `null` para distinguir semântica (“presente mas vazio” vs “ausente”) — caso **raro**; a **predefinição** do produto é **ausência**.  
...
**Proibição:** o pipeline de export **não** pode “renumerar” `media_id`, `scene_id` ou `draw_config_id` **só** porque é um novo arquivo — isso **quebra** binding local, relatórios de suporte e correspondência editor ↔ Pack. **Única** exceção: **migração** ou **correção** **explícita** com ADR e **novo** `pack_version`.
...
Quando o export for **entregue** como **arquivo único** (ex.: arquivo `.zip`), o nome **recomendado** **normativo** para **disciplina operacional** é:

`telaflow-pack-{event_slug}-{export_id}.{extensão}`

- **`event_slug`:** identificador **humano** estável e **seguro para arquivos** derivado do nome do evento (normalização definida em spec técnica ou UI — **sem** caracteres problemáticos para SO).  
...
| **Referência ao conjunto assinado** | Ligação explícita ao **manifesto de arquivos** ou **hash agregado** coberto por `signature.sig` (detalhe em spec técnica). |
...
Toda a **cadeia de confiança** (assinatura → arquivos → schemas → ids) **ancora** no `pack.json`: o operador e o suporte identificam **uma** linha de export; o Player decide **cedo** se o formato é **aceitável**; o Pre-flight (grupo **G1**) valida **integridade** e **coerência** com o que este manifesto **declara** ([PRE_FLIGHT_FEATURE_SPEC.md](./PRE_FLIGHT_FEATURE_SPEC.md)).
...
- Caminhos absolutos de disco, credenciais, dados de usuários da Cloud **não** necessários ao runtime.

## 8.3 O que precisa estar **congelado**

Qualquer campo que **influencie** o telão ou a **decisão** do operador no Player **deve** ser o **valor efetivo** no instante do export — em especial: ordem de Scenes, tipo de cada Scene, referências a mídia e sorteios, branding **resolvido** (arquivo dedicado mas **logicamente** parte do snapshot).
...
- Histórico de alterações de marca, rascunhos de tema, **assets** binários **não** previstos no contrato de branding do MVP (se no futuro houver arquivos **incluídos** no Pack, **exige** ADR — hoje a linha base **evita** mídia binária na Cloud/Pack para mídia operacional do cliente).
...
Todos os **DrawConfig** **referenciados** por **qualquer** Scene exportada **e** quaisquer outros **usos** previstos no schema (ex.: sorteio **não** ligado a Scene mas **executável** no roteiro — se o produto **permitir**, **deve** estar **explícito** no `event.json` ou regra de runtime documentada). Cada registro: identificador estável (`draw_config_id`), tipo de sorteio, parâmetros **necessários** ao Player conforme MVP, regras de **exibição pública** acordadas.
...
`media-manifest.json` é o **contrato declarativo** de tudo o que o operador **deve** (ou pode) colocar na **workspace** local. O Player **não** inventa slots; o **binding** associa arquivos a `media_id` ([ARCHITECTURE_SPEC.md](./ARCHITECTURE_SPEC.md) §8.3–8.4).
...
1. **Arquivos binários** de mídia do cliente (vídeo, imagem, áudio operacional).  
...
5. **Credenciais** de API, tokens de usuário, segredos de Cloud.  
6. **Logs de execução** de instalações anteriores do Player.  
7. **Caminhos absolutos** da máquina de qualquer usuário.  
...
- **Branding** obrigatório **ausente** quando a política do evento **exige** tema.  
- Falha de validação **JSON Schema** em qualquer artefato do Pack.  
- Incoerência entre **`event_id` / `organization_id`** no `pack.json` e o conteúdo dos demais arquivos.
...
- Metadados recomendados **ausentes** (ex.: duração sugerida **não** preenchida) — **não** elevam a bloqueante **salvo** regra de produto.
...
## 15.4 Ordem canônica e versionamento

Mudanças de **`pack_version`** ou de **schema** que **alterem** a **ordem canônica** de arquivos (§6.1) ou as **regras** de codificação (§6.2) / nullability (§6.3) são **breaking** **por padrão** e **exigem** ADR e atualização da matriz Player ↔ Pack.
...
O **conjunto** de artefatos **contratuais** acordado: tipicamente `pack.json` (metadados relevantes), `event.json`, `branding.json`, `media-manifest.json`, `draw-configs.json`, e o **corpo** de claims de licença — via **manifesto de arquivos** com hashes **ou** **serialização canônica** única ([ARCHITECTURE_SPEC.md](./ARCHITECTURE_SPEC.md) §9.3). A **ordem** em que os arquivos entram no manifesto de hashes **deve** seguir **estritamente** §6.1 (itens 1–6); cada entrada é processada com **UTF-8 sem BOM**, newlines **LF** e política de omissão de §6.2–6.3. **Não** assinar blobs de mídia **que não estão** no Pack; **opcionalmente** incluir hashes **esperados** no manifesto de mídia **se** existirem (política futura). O arquivo **`signature.sig`** **não** se auto-inclui no payload assinado.
...
No mínimo (conceitualmente): organização, evento, exportação (**`export_id`**), **validade temporal**, **escopo** de uso do Player. O Player **valida** correspondência entre claims e arquivos **e** assinatura ([ARCHITECTURE_SPEC.md](./ARCHITECTURE_SPEC.md) §10.4).
...
A **operação** de export **produz** **um** **Pacote** **identificável** (`export_id`) contendo **todos** os arquivos normativos §6 — quer **entregue** como diretório, arquivo contêiner ou transferência em stream, o **contrato** é **o mesmo**.
...
**Como artefato:** sim, **até** violar **validade** da licença **associada** ou **revogação** comercial **documentada** — o Player **valida** licença + assinatura no **momento** da carga. **Como “última verdade” na Cloud:** o **último** export **não** invalida **magicamente** arquivos **já** descarregados; a **governança** é **organizacional** (qual `export_id` **levar** ao palco).

## 19.3 **Como** diferenciar versões

Por **`export_id`**, **`generated_at`** e **`pack_version`**. A UI da Cloud **deve** listar histórico **legível** (UI_SPEC §6.2) — operador **nunca** depende só do nome de arquivo.
...
- **Quem** disparou o export (identidade de usuário).  
- **Quando** (`generated_at` **e** timestamp de registro servidor).  
...
12. **Ordem** de arquivos no manifesto de assinatura **arbitrária** ou **diferente** de §6.1.  
...
- **Novos artefatos** (ex.: `sponsors.json` dedicado): **entram** no **manifesto assinado** e no schema — **nunca** “arquivo extra” **não** coberto pela assinatura.  
